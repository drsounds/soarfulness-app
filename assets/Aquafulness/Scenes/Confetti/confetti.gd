...
func add_flake(flake):
	...
	var angle = randf_range(0, 179)
	var velocity_x = sin(angle / 180) * 10
	var velocity_z = cos(angle / 180) * 10

	flake.global_transform.origin = Vector3(
		rng.randf_range(
			bather_transform.origin.x - 500,
			bather_transform.origin.x + 500
		),
		bather_transform.origin.y + 200,
		rng.randf_range(
			bather_transform.origin.z - 500,
			bather_transform.origin.z + 500
		)
	)

	#flake.velocity = Vector3(velocity_x, 10, velocity_z)
	flake.visible = true
	flake.active = true
