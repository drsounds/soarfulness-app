...
func _process(delta:float) -> void:
	...
	if transform.origin.z < -BOUNDARY.x:
		transform.origin.z = BOUNDARY.z - 1
	"""
	var old_location = Vector3(location.x, location.y, location.z)
	...
	duo.transform.origin.y = -wave.y * 0.01

	"""
	if transform.origin.z < get_parent().swimmed_z_minus:
		get_parent().expand_forward()

	if transform.origin.z > get_parent().swimmed_z_plus:
		get_parent().expand_backward()

	if transform.origin.x > get_parent().swimmed_x_plus:
		get_parent().expand_right()

	if transform.origin.x < get_parent().swimmed_x_minus:
		get_parent().expand_left()
	"""
